...
class AIFoundryUtil:
    def __init__(self, endpoint: str, credential: AzureKeyCredential):
        ...
        self.mgmt_api_version = os.getenv(
            "AZURE_FOUNDRY_MGMT_API_VERSION", "2025-04-01-preview")
        
        self.project_client = None
        self.mgmt_client = None  # create as needed with 'initialize_mgmt_client()'

    def get_mgmt_client(self):
        try:
            if (self.mgmt_client) is None:
                subscription_id = os.getenv("AZURE_SUBSCRIPTION_NAME")
                self.mgmt_client = CognitiveServicesManagementClient(
                    credential=DefaultAzureCredential(), 
                    subscription_id=subscription_id,
                    api_version=self.mgmt_api_version)
        except Exception as e:
            print(f"Error initializing management client: {e}")
        return self.mgmt_client

    def create_project(self, resource_group_name: str, account_name: str, project_name: str) -> bool:
        try:
            # resource_group_name = os.getenv("AZURE_FOUNDRY_RG")
            # foundry_resource_name = os.getenv("AZURE_FOUNDRY_NAME")
            # foundry_project_name = os.getenv("AZURE_FOUNDRY_PROJECT_NAME")
