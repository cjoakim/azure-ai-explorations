...
def test_list_files_in_dir():
    ...
    files = FS.list_files_in_dir('../data/misc')
    assert str(type(files)) == "<class 'list'>" 
    print(files)
...
def test_read():
    s = FS.read("../data/misc/nc_zipcodes.json")
    assert len(s) == 304618
...
def test_read_lines():
    lines = FS.read_lines("../data/misc/postal_codes_nc.csv")
...
def test_read_json():
    obj = FS.read_json("../data/misc/nc_zipcodes.json")
    # print(obj)
...
def test_read_csv_as_rows_with_header():
    rows = FS.read_csv_as_rows("../data/misc/postal_codes_nc.csv")
...
def test_read_csv_as_rows_skip_header():
    rows = FS.read_csv_as_rows("../data/misc/postal_codes_nc.csv", skip=1)
...
def test_read_csv_as_dicts():
    rows = FS.read_csv_as_dicts("../data/misc/postal_codes_nc.csv")
    assert len(rows) == 1080
    assert rows[0]["postal_cd"] == "27006"
    assert rows[-1]["postal_cd"] == "28909"

def test_text_file_iterator():
    it = FS.text_file_iterator("../data/misc/postal_codes_nc.csv")
...
def test_walk():
    ...
    entries = FS.walk("../data", include_dirs=['../data/openflights/json'], include_types=['xml'])
    assert len(entries) == 0

    entries = FS.walk("../data", include_dirs=['../data/openflights/json'], include_types=['json'])
